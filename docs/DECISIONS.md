...
**Consequence** Commits from now on use the configured personal identity (no `-c user.email`). The repo was created **public**; the content licences allow it (Tatoeba CC BY, kaikki CC BY-SA, SAS CC BY-NC-SA, hermitdave frequency list CC BY-SA), but D118 had assumed private — flagged to the owner to decide. Production URL: https://learn-slovak-tau.vercel.app (Vercel project `learn-slovak`, personal scope, deployed by token only).
