...
## D137 — The 0.4 words join Flashcards and Speed match (2026-09-23)
**Decision** `CARD_KINDS` and `MATCH_KINDS` in `engine/pool.ts` now include `word`. Sound pairs stay out (0.2 is sound discrimination, nothing to recall); letters stay out of Speed match (their "meaning" is a sound anchor).
**Why** His report from 0.4: "in the exercise cards i can only include terms from 0.1 and 0.3 … i should be able to exercise all new words." The exclusion dated from D129, when a 0.4 word's gloss was the letter's sound anchor; D132 gave all 43 words a real gloss (checked: none missing) but the pool list was never updated.
**Consequence** Every completed 0.4 lesson feeds its words into both practice modes. Browser-checked: with 0.3 + 0.4 marked done the picker shows cognates 84 + words 43 and word cards render. The separate *Review* (Recapitulare) problem — it only ever holds FSRS sentence cards, which Phase 0 never creates — is diagnosed but not changed here; it needs his call on the design.
